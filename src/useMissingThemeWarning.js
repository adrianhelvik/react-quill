...
export default function useMissingThemeWarning({ options, noTheme }) {
  ...
  useEffect(() => {
    ...
    if (!theme) {
      console.warn(
        [
          'You should supply a theme to react-quill as in',
          '',
          "  <Quill options={{ theme: 'snow' }} />",
          '',
          'or explicitly specify that you do not want a theme',
          '',
          '  <Quill noTheme={true} />',
        ].join('\n'),
      )
